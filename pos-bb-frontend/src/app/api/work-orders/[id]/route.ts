...
export async function PATCH(
  ...
  try {
    ...
    const workOrder = await prisma.$transaction(async (tx) => {
      // 1. If services are provided, delete old records
      ...
      // 2. If parts are provided, adjust stock safely and delete old records
      if (result.data.parts !== undefined) {
        // Restore stock for existing parts first
        ...
        // Verify stock availability and deduct stock for new parts
        for (const p of result.data.parts) {
...
export async function DELETE(
  ...
  try {
    ...
    if (existing.invoice && existing.invoice.deletedAt === null) {
      return error("Cannot delete. This Work Order has already been invoiced.", 400);
    }

    const workOrder = await prisma.$transaction(async (tx) => {
      // Restore spare part stock for all active parts attached to this work order
      for (const p of existing.parts) {
        await tx.sparePart.update({
          where: { id: p.sparePartId },
          data: {
