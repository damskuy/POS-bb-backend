...
export const SortByDate: React.FC<SortByDateProps> = ({ order, onChange }) => {
  ...
  return (
    <div className="relative shrink-0" ref={dropdownRef}>
      ...
      >
        <span>Terdaftar: {order === "desc" ? "Terbaru" : "Terlama"}</span>
        <svg
      ...
      {isOpen && (
        <div className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-44 bg-white border border-slate-200/80 rounded-xl shadow-lg z-30 py-1.5 animate-fadeIn">
          <button
            onClick={() => {
              onChange("desc");
              setIsOpen(false);
            ...
            className={`w-full text-left px-3.5 py-2 text-xs font-semibold hover:bg-slate-50 transition-colors flex items-center justify-between ${
              order === "desc" ? "text-slate-900 bg-slate-50/50 font-bold" : "text-slate-600"
            }`}
          >
            <span>Terdaftar: Terbaru</span>
            {order === "desc" && (
          ...
          <button
            ...
            className={`w-full text-left px-3.5 py-2 text-xs font-semibold hover:bg-slate-50 transition-colors flex items-center justify-between ${
              order === "asc" ? "text-slate-900 bg-slate-50/50 font-bold" : "text-slate-600"
            }`}
          >
            <span>Terdaftar: Terlama</span>
            {order === "asc" && (
              <span className="w-1.5 h-1.5 bg-slate-900 rounded-full" />
            )}
          </button>
