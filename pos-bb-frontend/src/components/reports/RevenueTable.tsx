...
import { dailyTableData } from "@/mock/revenueReport";
          ...
          <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
            {dailyTableData.map((row) => (
              <tr key={row.date} className="hover:bg-slate-50/60 transition-colors">
                <td className="py-3.5 px-5 text-slate-500 font-semibold">{row.date}</td>
                <td className="py-3.5 px-4 text-center font-mono font-bold text-slate-900">{row.invoices}</td>
                <td className="py-3.5 px-4 text-center font-mono font-bold text-slate-900">{row.transactions}</td>
                <td className="py-3.5 px-4 text-right font-mono font-black text-slate-900">
                  {formatRupiah(row.revenue)}
                </td>
                <td className="py-3.5 px-5 text-right font-mono font-bold text-blue-600">
                  {formatRupiah(row.averageTicket)}
                </td>
              </tr>
            ))}
          </tbody>
