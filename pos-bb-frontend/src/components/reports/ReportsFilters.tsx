...
export const ReportsFilters: React.FC = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between pb-1">
      ...
      <div className="relative flex flex-col justify-end self-end mt-4 md:mt-0">
        <label className="hidden md:block text-[10px] font-bold text-transparent select-none mb-1">
          Aksi
        </label>
        <button
          type="button"
          onClick={() => setShowExportDropdown(!showExportDropdown)}
          className="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl transition-all shadow-xs flex items-center justify-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showExportDropdown && (
          <>
            ...
            />
            <div className="absolute right-0 bottom-full md:top-full md:bottom-auto mb-2 md:mb-0 md:mt-2 w-44 bg-white border border-slate-200 rounded-xl shadow-lg z-20 py-1 overflow-hidden animate-scaleUp">
              <button
                type="button"
                onClick={() => setShowExportDropdown(false)}
