...
interface ServiceContributionChartProps {
  contribution: ContributionItem[];
}

export const ServiceContributionChart: React.FC<ServiceContributionChartProps> = ({
  contribution,
}) => {
  const radius = 10;
  ...
  // Take top 5 and group remaining as "Others" if needed, but since database services are few, rendering all is fine.
  // Let's render the top 6.
  const displayContribution = contribution.slice(0, 6);
          ...
          {displayContribution.map((m, idx) => (
            <div key={idx} className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-700 truncate max-w-[100px]" title={m.category}>
                  {m.category}
                </span>
              </div>
              <div className="text-right shrink-0">
                <span className="font-bold text-slate-900 font-mono">{m.percentage}%</span>
                <span className="text-[9px] text-slate-400 font-medium block mt-0.5 font-mono">
