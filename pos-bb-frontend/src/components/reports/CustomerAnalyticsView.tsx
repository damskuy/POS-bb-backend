...
import { VisitFrequencyStats } from "./VisitFrequencyStats";
...
interface AnalyticsData {
  ...
  growth: Array<{ monthKey: string; label: string; count: number }>;
  topCustomers: Array<{
    customerId: number;
    name: string;
    phone: string;
  ...
  spendingDistribution: Array<{ bracket: string; count: number }>;
  visitFrequency: {
    averageVisits: number;
    oneTimeCustomers: number;
    repeatCustomers: number;
  };
  popularServices: Array<{ serviceName: string; totalUsage: number }>;
}
...
export const CustomerAnalyticsView: React.FC = () => {
  ...
  const handleExport = (format: "csv" | "excel") => {
    ...
    // Exporting top customers list
    const exportData = data.topCustomers.map((c) => ({
      "Customer Name": c.name,
      Phone: c.phone,
      "Total Visits": c.totalVisits,
      "Total Spending (Rp)": c.totalSpending,
      "Last Visit Date": c.lastVisit ? new Date(c.lastVisit).toLocaleDateString() : "-",
      "Average Ticket Size (Rp)": c.averageInvoice,
    }));
    ...
    link.setAttribute(
      "download",
      `customer_analytics_${new Date().toISOString().slice(0, 10)}.${format === "csv" ? "csv" : "xlsx"}`
    );
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px]">
        ...
        <p className="text-xs text-slate-500 font-semibold mt-4">Generating Customer Analytics...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white border border-slate-200/80 rounded-2xl px-5 py-4 shadow-xs">
        <div>
          <h4 className="text-xs font-bold text-slate-900">Retention & Behavior Analysis</h4>
          <p className="text-[10px] text-slate-400 mt-0.5">Metrics based on database transaction histories</p>
        </div>
      ...
      {/* Summary Metrics Row */}
      <CustomerAnalyticsSummary summary={data.summary} />

      {/* Growth & Spending Distribution Row */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2">
          <CustomerGrowthChart growth={data.growth} />
        </div>
        <div>
          <CustomerSpendingChart spendingDistribution={data.spendingDistribution} />
        </div>
      </div>

      {/* Visit Frequency & Popular Services Row */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <VisitFrequencyStats totalCustomers={data.summary.totalCustomers} visitFrequency={data.visitFrequency} />
        <PopularServicesChart popularServices={data.popularServices} />
      </div>

      {/* Top Customers & Inactive Customers Row */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <TopCustomersTable topCustomers={data.topCustomers} />
        <InactiveCustomersList inactiveCustomers={data.inactiveCustomers} />
      </div>
    </div>
  );
};
