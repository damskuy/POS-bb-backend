...
export const DeleteServiceDialog: React.FC<DeleteServiceDialogProps> = ({
  ...
  const handleDelete = async () => {
    setSubmitting(true);
    const success = await onConfirm(service.id);
    setSubmitting(false);
    if (success) {
      onClose();
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 z-10 space-y-4 animate-scaleUp">
        <div className="flex items-center gap-4 text-rose-600">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Hapus Jasa Servis</h3>
        ...
        <div className="p-3.5 bg-slate-50 border border-slate-200/60 rounded-xl text-xs text-slate-700 space-y-1">
          <p><span className="font-semibold text-slate-900">Nama Servis:</span> <strong className="text-slate-900">{service.name}</strong></p>
          <p><span className="font-semibold text-slate-900">Harga:</span> <span className="font-semibold text-emerald-600">{formatRupiah(service.price)}</span></p>
          {service.category && <p><span className="font-semibold text-slate-900">Kategori:</span> {service.category}</p>}
        </div>

        <p className="text-xs text-slate-600">
          Apakah Anda yakin ingin menghapus jasa servis <strong className="text-slate-900">{service.name}</strong>?
        </p>
