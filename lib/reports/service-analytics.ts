...
export async function getServiceAnalytics(): Promise<ServiceAnalyticsReport> {
  ...
  allWoServices.forEach((ws) => {
    ...
    const trendItem = dailyTrend.find((item) => {
      ...
      // rebuild date object from dailyTrend item label (simple check)
  ...
  Object.values(woServicesGroup).forEach((sIds) => {
    ...
    // Sort array to ensure key is consistent regardless of order (e.g. "1+2")
  ...
  // 9. Service Contribution Contribution toward total service revenue (Pie chart)
  ...
  allWoServices.forEach((ws) => {
    // Check if work order has a mechanic assigned
